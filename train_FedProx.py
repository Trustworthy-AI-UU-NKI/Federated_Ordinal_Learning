...
if __name__ == "__main__":
    ...
    if args.optimizer == "adam":
        lr_values = [1e-4]  # [3e-4, 1e-4, 1e-5]
    elif args.optimizer == "sgd":
        lr_values = [0.03, 1e-3, 1e-1]
    rounds_and_local_epochs = [(100, 1)]  # [(100, 1), (20, 5), (10, 10)]
    mus = [0.5]  # [0.001, 0.01, 0.1, 0.5, 1.0]
    losses = ["ordinal_encoding"]  # ["binomial", "ordinal_encoding", "ce"]
    grid_search = I.product(lr_values, rounds_and_local_epochs, mus, losses)
    ...
    for base_lr, round_and_local_epoch, mu, loss in grid_search:
        args.base_lr = base_lr
        args.rounds, args.local_ep = round_and_local_epoch
        args.mu = mu
        args.loss = loss
        outputs_dir = (
            ROOT_DIR / "results" / f"fedprox_fediic_{args.loss}_check"
        )  # ROOT_DIR / "results" / f"fedprox_fediic_parameters_study_{args.loss}"
        hyperparameter_setup = (
            f"{args.optimizer}_{args.base_lr}_{args.local_ep}_{args.rounds}_{args.mu}"
        )
            ...
            for fold in folds:
                ...
                for com_round in range(args.rounds):
                    ...
                    for id in range(args.n_clients):
                        ...
                        w, loss = local.train_fedprox(copy.deepcopy(net_local))
