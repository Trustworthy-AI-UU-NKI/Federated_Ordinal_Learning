...
from utils.local_training import LocalUpdateFedALA
...
def args_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="global learning rate, all experients in the oriinal paper are conducted with eta=1.0",
    )
...
if __name__ == "__main__":
    ...
    rounds_and_local_epochs = [(20, 5)]
    rand_percents = [80]
    layer_idxs = [2]
    losses = ["ordinal_encoding"]
    grid_search = I.product(
        lr_values, rounds_and_local_epochs, rand_percents, layer_idxs, losses
    )
    ...
    for base_lr, round_and_local_epoch, rand_percent, layer_idx, loss in grid_search:
        args.base_lr = base_lr
        args.rounds, args.local_ep = round_and_local_epoch
        args.rand_percent = rand_percent
        args.layer_idx = layer_idx
        args.loss = loss
        outputs_dir = ROOT_DIR / "results" / f"fedala_fediic_{args.loss}_check"
        hyperparameter_setup = f"{args.optimizer}_{args.base_lr}_{args.local_ep}_{args.rounds}_{args.rand_percent}_{args.layer_idx}_{args.batch_size}"
        for alpha, p in alpha_p_combinations:
            ...
            for fold in folds:
                ...
                net_glob = ResNet(args=args).to(device)
                print(net_glob)
                net_glob.train()
                ...
                for com_round in range(args.rounds):
                    ...
                    for id in range(args.n_clients):
                        ...
                        w, loss = local.train_fedala(copy.deepcopy(net_local))
                    ...
                    for id, client in tqdm(enumerate(trainer_locals)):
                        print(f"=========== Running ALA for client {id} ===========")
                        net_locals[id].load_state_dict(w_locals[id])
                        w_adapted = client.local_initialization(
                            net_locals[id], net_glob
                        )
                        net_locals[id].load_state_dict(w_adapted)
